...
import {
  PLAYER_COLORS, randomCallsign, SNAPSHOT_HZ, MAX_PLAYERS,
  RESPAWN_TIME, LTS_ROUND_WINS, PROTOCOL_VERSION,
...
import { createLoop, type GameLoop } from './engine/loop.js';
import { attachInput } from './engine/input.js';
import { sound } from './engine/sound.js';
import {
  MSG, hello, packSnapshot, unpackTankState,
  type HostMsg, type LobbyMsg, type SnapMsg,
} from './net/protocol.js';
import { HostLobby, GuestConnection, generateLobbyCode, joinURL } from './net/lobby.js';
import { qrDataURL } from './lib/qr.js';
import type {
  GameMode, GhostState, GhostTank, PlayerConfig, PlayerId, RenderShell, RenderState,
  RosterEntry, SimEvent, SimState, Standing,
} from './types.js';
...
const colorHex = (idx: number): string => PLAYER_COLORS[idx % PLAYER_COLORS.length].hex;

let myName = localStorage.getItem('bumper-tanks-name') || randomCallsign();
let session: LocalSession | GuestSession | null = null;
...
interface GuestCtx {
  ...
  myId: PlayerId | null;
  roster: RosterEntry[];
  conn: GuestConnection | null;
  rejected: boolean;
  settled: boolean; // welcomed, rejected, errored, or closed — stop the join timer
}

let host: HostCtx | null = null;
let guest: GuestCtx | null = null;
...
type ScreenName = 'menu' | 'lobby' | 'join' | 'results' | null;
...
function showScreen(name: ScreenName): void {
  ...
  if (name === 'menu') maybeApplyUpdate();
}

function setAccent(idx: number): void {
...
let transientMsg: { big: string; small: string | null; until: number } | null = null;
let scoreSig = '';

function centerMessage(big: string, small: string | null, dur = 1.6): void {
...
function nameSpan(tank: { name: string; colorIdx: number }): string {
...
function updateHUD(state: RenderState, myId: PlayerId | null): void {
  const timer = $('hud-timer');
  if (state.mode === 'rumble') timer.textContent = fmtTime(state.timeLeft);
  else if (state.mode === 'lts') timer.textContent = `R${state.round}`;
  ...
  const ranked = [...state.tanks].sort((a, b) =>
    state.mode === 'lts' ? b.roundWins - a.roundWins || b.score - a.score : b.score - a.score || a.deaths - b.deaths);
...
function processEvents(events: SimEvent[], state: RenderState, myId: PlayerId | null,
  { silent = false }: { silent?: boolean } = {}): void {
  for (const ev of events) {
    switch (ev.type) {
      case 'fire': {
        ...
        if (!silent) sound.fire();
        if (ev.id === myId && !reducedMotion.matches) camera.addShake(2.5);
        break;
      }
      case 'hit': {
        ...
        if (!silent) sound.hit();
        if (!reducedMotion.matches) camera.addShake(ev.targetId === myId ? 9 : 3.5);
        break;
      }
      case 'explode': {
      ...
      case 'bump':
        ...
        if (!silent) sound.bump(ev.intensity);
        break;
      case 'fall':
        if (!silent) sound.fall();
        break;
      case 'ko': {
        const victim = getTank(state, ev.victimId);
        const killer = ev.killerId != null ? getTank(state, ev.killerId) : undefined;
        if (victim) {
          if (killer) feed(`${nameSpan(killer)} ⌁ ${nameSpan(victim)}`);
          else feed(`${nameSpan(victim)} slipped into the void`);
        }
        if (!silent) sound.ko();
        if (ev.victimId === myId) centerMessage('INTO THE VOID', killer ? `${killer.name} got you` : 'nobody to blame', 1.8);
        else if (ev.killerId === myId && victim) centerMessage('+1', `${victim.name} knocked out`, 1.2);
        break;
      }
      case 'spawn': {
        const t = getTank(state, ev.id);
        if (t) particles.ring(ev.x, ev.y, colorHex(t.colorIdx), { radius: 6, growth: 320, life: 0.5, width: 4 });
        break;
      }
      case 'shieldPop':
        particles.ring(ev.x, ev.y, '#ffffff', { radius: 6, growth: 260, life: 0.25, width: 3 });
        if (!silent) sound.shieldPop();
        break;
      case 'countdown':
        centerMessage(String(ev.n), null, 0.85);
        if (!silent) sound.countdown(ev.n);
        break;
      case 'roundStart':
        centerMessage('GO', null, 0.8);
        if (!silent) sound.roundStart();
        break;
      case 'roundOver': {
...
interface LocalSessionOptions {
  ...
  attract?: boolean;
  playground?: boolean;
  onOver?: ((state: SimState) => void) | null;
}
...
class LocalSession {
  ...
  onOver: ((state: SimState) => void) | null;
  state: SimState;
  pendingNetEvents: SimEvent[] = [];
  snapTimer = 0;
  overFired = false;
  botCount: number;
  loop: GameLoop;

  constructor({ mode, mapId, players, myId, attract = false, playground = false, onOver = null }: LocalSessionOptions) {
    this.myId = myId;
  ...
  step(dt: number): void {
    ...
    if (this.attract) {
      ...
      for (const t of this.state.tanks) {
        if (t.state === 'active') { cx += t.x; cy += t.y; n++; }
      }
  ...
  render(alpha: number): void {
    if (!this.attract) updateHUD(this.state, this.myId);
  ...
  tap(sx: number, sy: number): void {
    if (this.attract || this.myId == null) return;
  ...
  addBot(): void {
    ...
    let idx = 0;
    while (used.has(idx) && idx < PLAYER_COLORS.length) idx++;
    this.botCount++;
    addTank(this.state, {
  ...
  removeBot(): void {
    ...
    if (bot) removeTank(this.state, bot.id);
  }
...
class GuestSession {
  ...
  state: GhostState;
  tanksById = new Map<PlayerId, GhostTank>();
  shellsById = new Map<number, RenderShell>();
  snapped = false;
  loop: GameLoop;

  constructor({ mode, mapId, roster, myId }: GuestSessionOptions) {
    this.myId = myId;
  ...
  onSnapshot(msg: SnapMsg): void {
    ...
    for (const row of msg.tanks) {
      ...
      if (info) { t.name = info.name; t.colorIdx = info.colorIdx; }
    }
    ...
    for (const [id, x, y, vx, vy, owner] of msg.shells) {
      ...
      if (!sh) {
        const fresh: RenderShell = { id, owner, x, y, px: x, py: y, vx, vy, radius: 7 };
  ...
  step(dt: number): void {
    ...
    if (s.mode === 'rumble' && s.phase === 'playing') s.timeLeft = Math.max(0, s.timeLeft - dt);
  ...
  tap(sx: number, sy: number): void {
    ...
    const me = this.tanksById.get(this.myId);
    if (me && me.state === 'active') me.tangle = Math.atan2(wy - me.y, wx - me.x);
  }

  updateRoster(roster: RosterEntry[]): void {
    this.roster = new Map(roster.map((p) => [p.id, p]));
  }
...
function showResults(standings: Standing[], winnerId: PlayerId | null, isHost: boolean): void {
  ...
  if (winner) setAccent(winner.colorIdx);
...
function standingsFrom(state: SimState): Standing[] {
  return [...state.tanks]
...
function refreshLobbyUI(): void {
  ...
  ($('btn-add-bot') as HTMLButtonElement).disabled = host.roster.length >= MAX_PLAYERS;
}
...
function startHosting(): void {
  ...
  host.lobby = new HostLobby(code, {
    onOpen: () => {
      ...
      ($('qr-img') as HTMLImageElement).src = qrDataURL(joinURL(code));
    },
    onError: (err) => {
    ...
    onLeave: (peerId) => {
      ...
      if (host.inMatch && session instanceof LocalSession) removeTank(session.state, peerId);
      broadcastLobby();
    },
    onMessage: (peerId, msg) => {
      ...
      if (msg.t === MSG.HELLO) {
        ...
        if (host.inMatch && session instanceof LocalSession) {
          ...
          if (session.state.mode === 'lts' && session.state.phase === 'playing') tank.state = 'ghost';
        ...
        if (host.inMatch && session instanceof LocalSession) applyTap(session.state, peerId, msg.x, msg.y);
...
function connectAsGuest(code: string, attempt = 0): void {
  endSession();
  startAttract();
  ...
  const ctx: GuestCtx = { code, myId: null, roster: [], conn: null, rejected: false, settled: false };
  guest = ctx;
  ...
  setTimeout(() => {
    if (ctx.settled || guest !== ctx) return;
    ctx.settled = true;
    ctx.conn?.destroy();
    guest = null;
  ...
  ctx.conn = new GuestConnection(code, {
    ...
    onError: (err) => {
      if (ctx.rejected || guest !== ctx) return; // resolved or superseded by a retry
      ctx.settled = true;
...
function showGuestLobby(lobbyMsg: LobbyMsg): void {
  if (!guest) return;
  $('lobby-code').textContent = guest.code;
  ($('qr-img') as HTMLImageElement).src = qrDataURL(joinURL(guest.code));
...
function startPlaygroundOn(mapId: string, botCount: number): void {
  endSession();
  const players: PlayerConfig[] = [{ id: 'me', name: myName, colorIdx: 0, isBot: false }];
...
$('btn-connect').addEventListener('click', () => {
  ...
  if (code.length >= 4) connectAsGuest(code);
});
$('join-code').addEventListener('keydown', (e) => {
  if ((e as KeyboardEvent).key === 'Enter') ($('btn-connect') as HTMLButtonElement).click();
});
...
$('mode-seg').addEventListener('click', (e) => {
  ...
  if (!btn || !host) return;
  host.mode = btn.dataset.mode as GameMode;
  for (const b of $('mode-seg').children) b.classList.toggle('on', b === btn);
  broadcastLobby();
});
...
$('map-seg').addEventListener('click', (e) => {
  const btn = (e.target as HTMLElement).closest('button[data-map]') as HTMLButtonElement | null;
  if (!btn || !host) return;
  host.mapChoice = btn.dataset.map!;
  for (const b of $('map-seg').children) b.classList.toggle('on', b === btn);
});
$('btn-add-bot').addEventListener('click', () => {
  if (!host || host.roster.length >= MAX_PLAYERS) return;
  const used = new Set(host.roster.map((p) => p.colorIdx));
  let idx = 0;
  while (used.has(idx)) idx++;
  host.roster.push({
...
$('pg-add-bot').addEventListener('click', () => {
  if (session instanceof LocalSession) session.addBot();
});
$('pg-remove-bot').addEventListener('click', () => {
  if (session instanceof LocalSession) session.removeBot();
});
...
function boot(): void {
  startAttract();
  if (joinFromHash()) return;
  if (location.hash === '#playground') startPlayground();
...
window.__game = {
  ...
  tapWorld(x: number, y: number) {
    if (session instanceof LocalSession && session.myId != null) applyTap(session.state, session.myId, x, y);
    else if (session instanceof GuestSession) guest?.conn?.send({ t: MSG.TAP, x, y });
  },
  startPlayground,
  leaveEverything,
  checkForUpdate,
