...
import type { MapDef, PlayerId, Rect, RenderState, RenderTank } from '../types.js';
...
function roundedRectPath(path: Path2D, rect: Rect): void {
...
export class Renderer {
  ...
  mapCache = new WeakMap<MapDef, MapPaths>();
  stars: Star[] = [];
  ...
  mapPaths(map: MapDef): MapPaths {
    let cached = this.mapCache.get(map);
    if (!cached) {
      const floors = new Path2D();
      for (const f of map.floors) roundedRectPath(floors, f);
      const holes = new Path2D();
      for (const h of map.holes) roundedRectPath(holes, h);
  ...
  render({ state, camera, particles, myId, alpha = 1, now = 0 }: RenderArgs): void {
    const ctx = this.ctx;
    const { vw, vh } = this;
    if (!this.bgGradient || !this.vignette) return;
    ...
    for (const tank of state.tanks) {
      if (tank.state === 'dead' || tank.state === 'ghost') continue;
  ...
  drawTank(ctx: CanvasRenderingContext2D, tank: RenderTank, alpha: number, now: number, isMe: boolean, px: number): void {
