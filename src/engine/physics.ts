...
import type { MapDef, Rect } from '../types.js';
...
export function sdfRoundedRect(rect: Rect, x: number, y: number): number {
...
export function pointInRoundedRect(rect: Rect, x: number, y: number): boolean {
  return sdfRoundedRect(rect, x, y) <= 0;
}

// Is a point standing on the map? Inside any floor and not inside any hole.
export function supported(map: MapDef, x: number, y: number): boolean {
  for (const h of map.holes) if (pointInRoundedRect(h, x, y)) return false;
  for (const f of map.floors) if (pointInRoundedRect(f, x, y)) return true;
  return false;
}
...
export function circleVsRoundedRect(x: number, y: number, radius: number, rect: Rect): WallHit | null {
  ...
  if (dist < 1e-6) {
    ...
    if (px < py) return { nx: Math.sign(dx) || 1, ny: 0, depth: px };
...
export function resolveCircleCircle(a: KineticBody, b: KineticBody, restitution: number): number {
  const dx = b.x - a.x;
  const dy = b.y - a.y;
...
export function bounceOffWall(body: KineticBody, rect: Rect, restitution: number): boolean {
...
export function angleLerpToward(current: number, target: number, maxDelta: number): number {
