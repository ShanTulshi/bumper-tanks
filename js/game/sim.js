...
import { TANK_RADIUS, SHELL_RADIUS, DAMPING, ROT_SPEED, FIRE_COOLDOWN, RECOIL_IMPULSE, SHELL_SPEED, SHELL_RANGE, HIT_IMPULSE, AOE_RADIUS, AOE_IMPULSE, SELF_HIT_GRACE, TANK_RESTITUTION, WALL_RESTITUTION, BUMP_MIN_SPEED, FALL_GRACE_RATIO, FALL_TIME, RESPAWN_TIME, SPAWN_SHIELD, KILL_CREDIT_WINDOW, RUMBLE_DURATION, LTS_ROUND_WINS, COUNTDOWN, ROUND_END_PAUSE, MATCH_END_PAUSE, } from './constants.js';
import { mapById } from './maps.js';
import { supportedWithGrace, resolveCircleCircle, bounceOffWall, angleLerpToward, angleDiff, } from '../engine/physics.js';
export function createMatch({ mode, mapId, players }) {
    const state = {
...
function recordHit(state, victim, attackerId) {
    victim.lastHitBy = attackerId;
    victim.lastHitT = state.time;
}
function explode(state, x, y, ownerId) {
    state.events.push({ type: 'explode', x, y, owner: ownerId });
    for (const tank of state.tanks) {
        if (tank.state !== 'active' || tank.shield > 0)
            continue;
        const dx = tank.x - x;
        const dy = tank.y - y;
        ...
        if (tank.id !== ownerId)
            recordHit(state, tank, ownerId);
    }
}
function fireShell(state, tank) {
...
export function step(state, dt) {
    ...
    if (state.phase === 'countdown') {
        ...
        if (after < before && after > 0)
            state.events.push({ type: 'countdown', n: after });
        if (state.phaseT <= 0) {
            state.phase = 'playing';
            state.phaseT = 0;
        ...
        if (state.phaseT <= 0) {
            state.round += 1;
            state.tanks.forEach((t) => { if (t.state === 'ghost')
                t.state = 'active'; });
            placeAtSpawns(state);
        ...
        if (after < before && after <= 5 && after > 0)
            state.events.push({ type: 'countdown', n: after });
        if (state.timeLeft <= 0) {
            state.timeLeft = 0;
    ...
    for (let i = 0; i < state.tanks.length; i++) {
        for (let j = i + 1; j < state.tanks.length; j++) {
            ...
            if (impact > BUMP_MIN_SPEED) {
                if (a.shield <= 0)
                    recordHit(state, a, b.id);
                if (b.shield <= 0)
                    recordHit(state, b, a.id);
                state.events.push({
                    type: 'bump',
                    x: (a.x + b.x) / 2,
                    y: (a.y + b.y) / 2,
    ...
    for (const tank of state.tanks) {
        ...
        for (const wall of state.map.walls)
            bounceOffWall(tank, wall, WALL_RESTITUTION);
    }
    // --- Shells -------------------------------------------------------------
    const deadShells = new Set();
    for (const shell of state.shells) {
        ...
        for (const tank of state.tanks) {
            if (tank.state !== 'active')
                continue;
            if (tank.id === shell.owner && shell.age < SELF_HIT_GRACE)
                continue;
            const d = Math.hypot(tank.x - shell.x, tank.y - shell.y);
            if (d > tank.radius + shell.radius)
                continue;
            if (tank.shield > 0) {
                ...
                if (tank.id !== shell.owner)
                    recordHit(state, tank, shell.owner);
    ...
    if (deadShells.size)
        state.shells = state.shells.filter((s) => !deadShells.has(s.id));
