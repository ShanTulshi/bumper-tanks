...
const colorName = (idx) => PLAYER_COLORS[idx % PLAYER_COLORS.length].name;

let myName = localStorage.getItem('bumper-tanks-name') || randomCallsign();
let session = null; // active game/attract driver
let host = null; // HostLobby context
let guest = null; // GuestConnection context
...
let transientMsg = null; // {big, small, until}
let scoreSig = '';
...
function updateHUD(state, myId) {
  ...
  if (transientMsg && now < transientMsg.until) {
    ...
    centerEl.dataset.key = transientMsg.big + (transientMsg.small || '');
  } else {
    transientMsg = null;
    const me = getTank(state, myId);
    if (me && me.state === 'dead') {
...
function processEvents(events, state, myId, { silent = false } = {}) {
  for (const ev of events) {
    switch (ev.type) {
      ...
      case 'explode': {
        const t = ev.owner != null ? getTank(state, ev.owner) : null;
      ...
      case 'ko': {
        ...
        const killer = ev.killerId != null ? getTank(state, ev.killerId) : null;
        if (victim) {
      ...
      case 'roundStart':
        ...
        break;
      case 'roundOver': {
        const w = ev.winnerId != null ? getTank(state, ev.winnerId) : null;
...
class LocalSession {
  constructor({ mode, mapId, players, myId, attract = false, playground = false, onOver = null }) {
    ...
    particles.clear();
    const me = getTank(this.state, myId);
...
class GuestSession {
  ...
  onSnapshot(msg) {
    ...
    for (const [id, x, y, vx, vy, owner] of msg.shells) {
      seenShells.add(id);
      let sh = this.shellsById.get(id);
      if (!sh) {
        sh = { id, owner, x, y, px: x, py: y, vx, vy, radius: 7 };
        this.shellsById.set(id, sh);
        s.shells.push(sh);
      } else {
...
function showResults(standings, winnerId, isHost) {
  ...
  if (winner) setAccentOfPlayer(winner);
  ...
  function setAccentOfPlayer(p) {
    // Results glow in the winner's color — a little coronation.
    document.documentElement.style.setProperty('--accent', colorHex(p.colorIdx));
  }
}
...
function refreshLobbyUI() {
  ...
  $('lobby-roster').innerHTML = host.roster.map((p) => `
    <div class="roster-row">
      <div class="chip" style="color:${colorHex(p.colorIdx)};background:${colorHex(p.colorIdx)}"></div>
      <div>${p.name}</div>
      <div class="who">${p.isBot ? 'bot' : p.id === 'host' ? 'host · you' : p.id === guestSelfId() ? 'you' : 'pilot'}</div>
    </div>`).join('');
  const canStart = host.roster.filter((p) => !p.isBot).length >= 1 && host.roster.length >= 2;
  $('btn-start').disabled = !canStart;
  $('btn-start').textContent = canStart ? 'Launch' : 'Need 2+ tanks';
  $('btn-add-bot').disabled = host.roster.length >= MAX_PLAYERS;
}

function guestSelfId() { return guest?.myId; }
...
function startHosting() {
  ...
  host.lobby = new HostLobby(code, {
    onOpen: () => {
      ...
      const img = $('qr-img');
      img.src = qrDataURL(joinURL(code));
    },
    onError: (err) => {
      ...
      $('lobby-status').textContent = `network trouble: ${err.type || err}`;
      $('lobby-status').classList.add('error');
    },
    onJoin: () => {},
    onLeave: (peerId) => {
      host.roster = host.roster.filter((p) => p.id !== peerId);
      if (host.inMatch && session) removeTank(session.state, peerId);
      broadcastLobby();
    },
    onMessage: (peerId, msg) => {
      if (msg.t === MSG.HELLO) {
        ...
        if (session && host.inMatch) applyTap(session.state, peerId, msg.x, msg.y);
...
function connectAsGuest(code) {
  ...
  guest.conn = new GuestConnection(code, {
    onOpen: () => {
      guest.conn.send(hello(myName));
      $('join-status').textContent = 'joining lobby…';
    },
    onError: (err) => {
      const reason = err.type === 'peer-unavailable' ? 'no arena with that code' : `connection failed: ${err.type || err}`;
    ...
    onMessage: (msg) => {
      if (msg.t === MSG.WELCOME) {
        ...
        guest.roster = msg.roster;
        const me = msg.roster.find((p) => p.id === guest.myId);
        ...
        endSession();
        showScreen(null);
        session = new GuestSession({ mode: msg.mode, mapId: msg.mapId, roster: msg.roster, myId: guest.myId });
...
$('name-input').addEventListener('change', () => {
  const name = $('name-input').value.trim().toUpperCase().slice(0, 10) || myName;
  myName = name;
  $('name-input').value = name;
  localStorage.setItem('bumper-tanks-name', name);
  if (host) {
    host.roster.find((p) => p.id === 'host').name = name;
    broadcastLobby();
  } else if (guest?.conn) {
...
// Playground bar.
$('pg-add-bot').addEventListener('click', () => session instanceof LocalSession && session.addBot());
$('pg-remove-bot').addEventListener('click', () => session instanceof LocalSession && session.removeBot());
$('pg-map').addEventListener('click', () => {
  if (!(session instanceof LocalSession)) return;
  const cur = MAPS.findIndex((m) => m.id === session.state.mapId);
  const next = MAPS[(cur + 1) % MAPS.length];
  const bots = session.state.tanks.filter((t) => t.isBot).length;
  startPlaygroundOn(next.id, bots);
});
...
window.__game = {
  ...
  tapWorld(x, y) {
    if (session instanceof LocalSession) applyTap(session.state, session.myId, x, y);
